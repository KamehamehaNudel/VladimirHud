...
enum InventoryMenuTabIndexes
{
	...
	InventoryMenuTab_Armor = 1,
	InventoryMenuTab_Potions = 2,
	InventoryMenuTab_OilsBombs = 3,
	InventoryMenuTab_Ingredients = 4,
	InventoryMenuTab_Books = 5,
	InventoryMenuTab_QuestItems = 6,
	InventoryMenuTab_Default = 7,
	InventoryMenuTab_Horse = 8
	// ---=== VladimirHUD ===---
};
...
class CR4InventoryMenu extends CR4MenuBase
{
	...
	// ---=== VladimirHUD ===--- Lim3zer0 && KamehamehaNudel
	private var m_fxEnableHorsePaperdoll	 : CScriptedFlashFunction;
	private var m_fxStatsRing				 : CScriptedFlashSprite;
	private var m_fxItemIcon				 : CScriptedFlashFunction;
	...
	event  OnConfigUI()
	{
		...
		// ---=== VladimirHUD ===--- Lim3zer0 && KamehamehaNudel
		m_fxEnableHorsePaperdoll = m_flashModule.GetMemberFlashFunction( "enableHorsePaperdoll" );
		m_fxStatsRing = m_flashModule.GetChildFlashSprite("vHudStatsRing");
		m_fxItemIcon = m_flashModule.GetChildFlashSprite("moduleMerchantInfo").GetMemberFlashFunction("loadItem");
		...
		m_fxSetNewFlagsForTabs.InvokeSelfNineArgs(
			FlashArgBool(hasNewItems[0]),
			FlashArgBool(hasNewItems[1]),
			FlashArgBool(hasNewItems[2]),
			FlashArgBool(hasNewItems[3]),
			FlashArgBool(hasNewItems[4]),
			FlashArgBool(hasNewItems[5]),
			FlashArgBool(hasNewItems[6]),
			FlashArgBool(hasNewItems[7]),
			FlashArgBool(hasNewItems[8])
			);
		...
		// ---=== VladimirHUD ===--- Lim3zer0
		m_fxEnableHorsePaperdoll.InvokeSelfOneArg(FlashArgBool(currentlySelectedTab == InventoryMenuTab_Horse));
		// ---=== VladimirHUD ===---
	...
	// ---=== VladimirHUD ===--- Erx
	private function RefreshGUIHorseEquipment()
	{		
		var id, allItems, allItemsGUIHorse : array<SItemUniqueId>;
		var i : int;
		var realHorseInv, guiHorseInv : CInventoryComponent;	
		var ent : CEntity;
		...
		guiHorseInv.GetAllItems(allItemsGUIHorse);	

		for(i=0;i<allItemsGUIHorse.Size();i+=1)
		{
			ent = guiHorseInv.GetItemEntityUnsafe(allItemsGUIHorse[i]);
	...
	event  OnGuiSceneEntitySpawned(entity : CEntity)
	{
		...
		// ---=== VladimirHUD ===--- Lim3zer0		
		if( entity.GetReadableName() == "characters\player_entities\geralt\geralt_player.w2ent" )
		{
			m_player = entity;
			m_horse = NULL;
		}
		else if( entity.GetReadableName() == "characters\npc_entities\animals\horse\player_horse.w2ent" )
		{
			m_player = NULL;
			m_horse = entity;	
		}
	...
	event  OnResetPlayerPosition()
	{
		...
		// ---=== VladimirHUD ===--- Lim3zer0
		guiSceneController.ResetEntityPosition(drawHorse);
		// ---=== VladimirHUD ===---
	}
	...
	event  OnTabChanged(tabIndex:int)
	{
		...
		{
			if( tabIndex == InventoryMenuTab_Horse && currentlySelectedTab != InventoryMenuTab_Horse )
			{
				m_fxEnableHorsePaperdoll.InvokeSelfOneArg(FlashArgBool(true));
				m_fxForceSelectPaperdollSlot.InvokeSelfOneArg(FlashArgInt(26));
				drawHorse = true;
				UpdateEntityTemplate();	
			}
			else if( currentlySelectedTab == InventoryMenuTab_Horse && tabIndex != InventoryMenuTab_Horse )
			{
				m_fxEnableHorsePaperdoll.InvokeSelfOneArg(FlashArgBool(false));
				m_fxForceSelectPaperdollSlot.InvokeSelfOneArg(FlashArgInt(0));
				drawHorse = false;
				UpdateEntityTemplate();
			}
		}
	...
	{
		showNotification(currentlySelectedTab);
	...
	public function PopulateTabData(tabIndex:int) : void
	{
		...
		l_flashArray = m_flashValueStorage.CreateTempFlashArray();
			...
			case InventoryMenuTab_Weapons:
				...
				break;
			case InventoryMenuTab_Armor:
				_playerInv.SetFilterType( IFT_Armors );
				_playerInv.GetInventoryFlashArray(l_flashArray, l_flashObject);
				break;
			case InventoryMenuTab_Potions:
				...
				_playerInv.GetInventoryFlashArray(l_flashArray, l_flashObject);
				break;
			case InventoryMenuTab_OilsBombs:
				_playerInv.SetFilterType( IFT_OilsBombs );
				_playerInv.GetInventoryFlashArray(l_flashArray, l_flashObject);
				break;
			...
			case InventoryMenuTab_Default:
				...
				_playerInv.GetInventoryFlashArray(l_flashArray, l_flashObject);
				break;
			case InventoryMenuTab_Horse:
				_playerInv.SetFilterType( IFT_HorseItems );
				_playerInv.GetInventoryFlashArray(l_flashArray, l_flashObject);
				break;
			}
			// ---=== VladimirHUD ===---
	...
	public function SetInitialTabNewFlags( out hasNewItems : array< bool > ):int
	{
		...
		_playerInv.SetFilterType( IFT_Books );
		hasNewItems.PushBack( _playerInv.HasNewFlagOnItem( hasItems ) );
		if (hasItems) notEmptyTabIdx = 5;
		
		_playerInv.SetFilterType( IFT_QuestItems );
		hasNewItems.PushBack( _playerInv.HasNewFlagOnItem( hasItems ) );
		if (hasItems) notEmptyTabIdx = 6;
		
		_playerInv.SetFilterType( IFT_Default );
		hasNewItems.PushBack( _playerInv.HasNewFlagOnItem( hasItems ) );
		if (hasItems) notEmptyTabIdx = 7;
		
		_playerInv.SetFilterType( IFT_HorseItems );
		hasNewItems.PushBack( _playerInv.HasNewFlagOnItem( hasItems ) );
		if (hasItems) notEmptyTabIdx = 8;
		// ---=== Lim3zer0 ===---
	...
	public function SetTabNewFlags():void
	{
		...
		m_fxSetNewFlagsForTabs.InvokeSelfNineArgs
		(
			FlashArgBool(hasNewItems[IFT_Weapons]), 
			FlashArgBool(hasNewItems[IFT_Armors]), 
			FlashArgBool(hasNewItems[IFT_AlchemyItems]), 
			FlashArgBool(hasNewItems[IFT_OilsBombs]), 
			FlashArgBool(hasNewItems[IFT_Ingredients]),
			FlashArgBool(hasNewItems[IFT_Books]),
			FlashArgBool(hasNewItems[IFT_QuestItems]),
			FlashArgBool(hasNewItems[IFT_Default]),
			FlashArgBool(hasNewItems[IFT_HorseItems])
		);
		// ---=== VladimirHUD ===---
	}
	...
	public function getTabFromFilter(inventoryFilterType:EInventoryFilterType):int
	{
		...
		switch (inventoryFilterType)
		{
			...
			case IFT_Armors:
				return InventoryMenuTab_Armor;
			case IFT_AlchemyItems:
				return InventoryMenuTab_Potions;
			case IFT_OilsBombs:
				return InventoryMenuTab_OilsBombs;
	...
	private function SetInventoryState(targetMode : int) : void
	{
		...
		switch (targetMode)
		{
			case IMS_Player:
				...
				m_fxEnableHorsePaperdoll.InvokeSelfOneArg(FlashArgBool(false));
				m_fxForceSelectPaperdollSlot.InvokeSelfOneArg(FlashArgInt(0));
				m_flashModule.GetChildFlashSprite( "vHudStatsRing" ).SetVisible( true );
				// ---=== VladimirHUD ===---
				break;
			case IMS_Shop:
			    // ---=== VladimirHUD ===--- PaulScottt && KamehamehaNudel
				UpdateShop();
				m_flashModule.GetChildFlashSprite( "vHudStatsRing" ).SetVisible( false );
				m_fxSetFilteringMode.InvokeSelfOneArg(FlashArgBool(false));
				PlayerMoney();
				// ---=== VladimirHUD ===--- 
				break;
	...
	private function UpdateEntityTemplate() : void
	{
		...
		if ( drawHorse )
		{
			...
			appearance 					 = GetWitcherPlayer().GetHorseManager().RequestGUIHorseAppearance();
			environmentSunRotation.Yaw   = 250;
			environmentSunRotation.Pitch = 10;
			cameraLookAt.Z               = 0.92;
			cameraLookAt.X               = 0.4;
			cameraRotation.Yaw           = 220;
		...
		else
		{
			...
			appearance                   = '';
			environmentSunRotation.Yaw   = 90;
			environmentSunRotation.Pitch = 0;
			cameraLookAt.Z               = 0.92;
			cameraRotation.Yaw           = 145; 
		...
		// ---=== VladimirHUD ===--- Lim3zer0
		guiSceneController.SetEnvironmentAndSunRotation( "VUILighting", environmentSunRotation );
		// ---=== VladimirHUD ===---
	...
	// ---=== VladimirHUD ===--- Lim3zer0
	private function UpdateStatsRing()
	{
		var sp 								: SAbilityAttributeValue;
		var sign, attack, armor, vitality 	: string;
		var textPrefix 						: string = "<font size='15' color='#ffffff' face=\"$NormalFont\">";
		var valueAbility					: float;
		var curStats						: SPlayerOffenseStats;
		var steel, silver					: int;
		...
		sign = "+" + (string)RoundMath(valueAbility * 100) + " %";
		
		valueAbility =  CalculateAttributeValue( GetWitcherPlayer().GetTotalArmor() );
		armor = IntToString( RoundMath(  valueAbility ) );
		
		vitality = (string)RoundMath(thePlayer.GetStatMax(BCS_Vitality));
		
		curStats = GetWitcherPlayer().GetOffenseStatsList();
		steel = RoundMath((curStats.steelFastDPS+curStats.steelStrongDPS)/2);
		silver = RoundMath((curStats.silverFastDPS+curStats.silverStrongDPS)/2);
		
		attack = (string)((steel+silver)/2);
		
		m_fxStatsRing.GetChildFlashTextField( "textVitality" ).SetTextHtml( textPrefix + StrUpper(GetLocStringByKeyExt("vitality")) + "</font>" );
		m_fxStatsRing.GetChildFlashTextField( "textSign" ).SetTextHtml( textPrefix + StrUpper(GetLocStringByKeyExt("stat_signs")) + "</font>" );
		m_fxStatsRing.GetChildFlashTextField( "textAttack" ).SetTextHtml( textPrefix + "ATTACK</font>" );
		m_fxStatsRing.GetChildFlashTextField( "textArmor" ).SetTextHtml( textPrefix + StrUpper(GetLocStringByKeyExt("attribute_name_armor")) + "</font>" );
		m_fxStatsRing.GetChildFlashTextField( "valueVitality" ).SetTextHtml( textPrefix + vitality + "</font>" );
		m_fxStatsRing.GetChildFlashTextField( "valueSign" ).SetTextHtml( textPrefix + sign + "</font>" );
		m_fxStatsRing.GetChildFlashTextField( "valueAttack" ).SetTextHtml( textPrefix + attack + "</font>" );
		m_fxStatsRing.GetChildFlashTextField( "valueArmor" ).SetTextHtml( textPrefix + armor + "</font>" );
	}
	
	// ---=== VladimirHUD ===--- KamehamehaNudel & Paulscott
	
	private function PlayerMoney() 
	{
		var textPrefix 						: string = "<font size='19' color='#ffffff' face=\"$NormalFont\">";
		var m_MerchantInfo 					: CScriptedFlashSprite;
		var Money 							: int;
		
		
		m_MerchantInfo = m_flashModule.GetChildFlashSprite("moduleMerchantInfo");
		Money = thePlayer.GetMoney();
		
		m_MerchantInfo.GetChildFlashTextField( "playerMoney" ).SetTextHtml ( textPrefix + IntToString(Money) + "</font>");
		m_MerchantInfo.GetChildFlashTextField( "txtGeralt" ).SetTextHtml ( "<font size='29' color='#ffffff'>" + StrUpper(GetLocStringByKeyExt("gwint_geralt")) + "</font>");
	    m_MerchantInfo.GetChildFlashTextField( "txtOrens1" ).SetTextHtml ( "<font letterSpacing='0.500000' size='18'>" + "ORENS" + "</font>");
	    m_MerchantInfo.GetChildFlashTextField( "txtOrens2" ).SetTextHtml ( "<font letterSpacing='0.500000' size='18'>" + "ORENS" + "</font>");
	}
	...
	public function ShowItemIcon(item : SItemUniqueId)
	{
		var imgPath					: string;
		var itemName				: name;
		var itemType				: EInventoryFilterType;
		var gridSize				: int;
		var _shop_inv				: CInventoryComponent;
		
		_shop_inv = _shopNpc.GetInventory();
		
		if(_shop_inv.GetItemIconPathByUniqueID(item) != "")
		{
			imgPath = _shop_inv.GetItemIconPathByUniqueID(item);
			if (_shop_inv.IsItemTrophy( item ) || _shop_inv.IsItemHorseItem(item))
		    {
		    	gridSize = 1;
		    }
		    else
		    {
		    	gridSize = _shop_inv.GetItemGridSize(item);
		    }
		}
		else
		{
			imgPath = _inv.GetItemIconPathByUniqueID(item);
		    if (_inv.IsItemTrophy( item ) || _inv.IsItemHorseItem(item))
		    {
		    	gridSize = 1;
		    }
		    else
		    {
		    	gridSize = _inv.GetItemGridSize(item);
		    }
		}
		
		
		m_fxItemIcon.InvokeSelfTwoArgs(FlashArgString(imgPath), FlashArgInt(gridSize));
	...
	private function UpdateGuiSceneEntityItems()
	{
		...
		if( !drawHorse )  // ---=== VladimirHUD ===--- KamehamehaNudel
		{
